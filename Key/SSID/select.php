...
if ($ApiFunction->Check_Ukey($GetData['ukey'])) {
    ...
    if (password_verify($GetData['password'],$Result_User_Object->password)) {
        ...
        if ($Result_SSID_Object->id != null) {
            ...
            $data = array(
                ...
                'info'=>'数据查询完毕',
                'data'=>[
                    'ssid'=>$Result_SSID_Object->ssid,
                ]
            );
            // 输出数据
            $ApiFunction->logs('ssid_select','数据查询',1);
        } else {
            // 编译数据
            $data = array(
                'output'=>'SSID_NONE',
            ...
            // 输出数据
            $ApiFunction->logs('ssid_select','数据查询',0,'Server_SSIDNone');
        }
    } else {
        // 编译数据
        $data = array(
        ...
        // 输出数据
        $ApiFunction->logs('ssid_select','密码验证',0,'Query_Password');
    ...
    // 输出数据
    $ApiFunction->logs('ssid_select','密钥检查',0,'Query_Ukey');
    header("HTTP/1.1 403 Forbidden");
}
